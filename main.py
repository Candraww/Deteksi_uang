...
def capture_and_predict_loop(model, label_map, output_path="dataset/test/test_capture.jpg", contrast_alpha=2.0, contrast_beta=50):
    ...
    while True:
        ...
        if ret:
            ...
            if key == ord(' '):
                ...
                # Predict the nominal value of the captured image
                predicted_label = predict_image(model, label_map, output_path, contrast_alpha=contrast_alpha, contrast_beta=contrast_beta)
                if predicted_label is not None:
                    predicted_nominal = label_map[predicted_label]
                    display_prediction(output_path, predicted_nominal)
...
if __name__ == "__main__":
    ...
    # Capture image from camera, predict nominal, and loop until 'Esc' is pressed
    capture_and_predict_loop(model, label_map, output_path="dataset/test/test_capture.jpg", contrast_alpha=2.0, contrast_beta=50)
